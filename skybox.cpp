...
int main() {
    ...
    if (loadOBJWithMTL("data/chair.obj", verticesArray, indicesArray, textureIDs, texturePaths)) {
        ...
        for (size_t i = 0; i < verticesArray.size(); i++) {
            ...
            if (i < texturePaths.size() && !texturePaths[i].empty()) {
                mesh.textureName = texturePaths[i];
                
                // Try to load texture from various locations
                ...
                for (const auto& path : possiblePaths) {
                    mesh.textureID = loadTexture(path.c_str());
                    if (mesh.textureID > 0) {
                        std::cout << "Loaded texture: " << path << " for mesh " << i << std::endl;
                        break;
                    }
                }
            } else {
                mesh.textureName = "default";
                mesh.textureID = 0;
            }
            ...
            // Vertex attributes (position, color, normal, texcoord)
    ...
    while (!glfwWindowShouldClose(window)) {
       ...
       glUseProgram(chairShader);
        ...
        glUniform3f(glGetUniformLocation(chairShader, "lightPos"), lightPos.x, lightPos.y, lightPos.z);
        glUniform3f(glGetUniformLocation(chairShader, "viewPos"), cameraPos.x, cameraPos.y, cameraPos.z);
        glUniform3f(glGetUniformLocation(chairShader, "lightColor"), lightColor.x, lightColor.y, lightColor.z);
        ...
        for (const auto& mesh : chairMeshes) {
            ...
            if (mesh.textureID > 0) {
                ...
                glUniform1i(glGetUniformLocation(chairShader, "texture1"), 0);
            } else {
            ...
            if (mesh.indices.empty()) {
                glDrawArrays(GL_TRIANGLES, 0, mesh.vertices.size() / 11);
            } else {
...
bool loadOBJWithMTL(const char* objPath, 
    ...
    std::map<std::string, std::map<std::string, unsigned int>> mtlVertexMap; // Track unique vertices per material
    ...
    while (std::getline(objFile, line)) {
        ...
        if (prefix == "usemtl") {
            ...
            // For quads, split into two triangles: (0,1,2) and (0,2,3)
            ...
            for (int t = 0; t < triangles; t++) {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    // Create a unique key for vertex combination
